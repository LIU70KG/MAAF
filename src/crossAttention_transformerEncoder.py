...
class EncoderLayer(nn.Module):
    def __init__(self, d_model, ffn_hidden, n_head, drop_prob=0.1):
        ...
        self.attention = nn.MultiheadAttention(embed_dim=d_model, num_heads=n_head, batch_first=True)
        # self.attention = MultiHeadAttention(d_model=d_model, n_head=n_head)#这是教程上编写的多头注意力，这里用自带的nn.MultiheadAttention代替了
    ...
    def forward(self, x,kv):
        ...
        _x = x
        attn_output, attn_output_weights = self.attention(x, kv, kv)
        x = attn_output
